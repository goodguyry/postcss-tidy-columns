...
const { typicalWithBreakpoints } = require('../test/sharedConfigs');
...
describe('Test `tidy-var` function replacement', () => {
  ...
  test(
    ...
    () => run(
      '@media (min-width: 1280px) { div { max-width: tidy-var(siteMax); } }',
      '@media (min-width: 1280px) { div { max-width: 90rem; } }',
    ),
  );
