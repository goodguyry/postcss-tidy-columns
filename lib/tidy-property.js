 ...
 * @returns {Object}
 */
function cleanClone(declaration, overrides = {}) {
...
module.exports = function tidyProperty(declaration, tidy) {
  ...
  if (OFFSET_REGEX.test(declaration.prop)) {
    ...
    const fluidDecl = cleanClone(declaration, {
    ...
    if (undefined !== full) {
      ...
      const fullDecl = cleanClone(declaration, {
        prop: `margin-${direction}`,
        value: full,
      });
      ...
      if ('rule' === fullWidthRule.type) {
        // Normal rule.
        fullWidthRule.source = declaration.source;
