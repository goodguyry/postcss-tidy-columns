    ...
    .reduce((acc, key) => {
      ...
      if (varPattern.test(option)) {
        ...
        if ('breakpoints' === key) {
          ...
          const sortedBreakpoints = sortObjectsByProperty(breakpoints, 'breakpoint');

          // Merge each config's smaller sibling into it to mimic the cascade.
          if (1 < sortedBreakpoints.length) {
            breakpoints = sortedBreakpoints.reverse().map((config, index) => {
              // Close the sibling config.
              const nextConfig = Object.assign({}, sortedBreakpoints[index + 1]);
